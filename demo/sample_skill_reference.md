**This is a copy of the real, shipped `skill/references/iclr.md`** — not a hand-curated
demo. It's the actual output of `paperscope build-skill` against this project's real
fetched ICLR corpus (`data/full/iclr.jsonl`, gitignored) and the claims in
[`../artifacts/claims.json`](#) (not committed — regenerate with
[`docs/skill_building.md`](../docs/skill_building.md)'s workflow). `sample_skill_manifest.json`
alongside this file is the matching `skill/manifest.json` — note `"preliminary": true`
and `"decisions_resolved": 0`. See [`sample_reference.md`](sample_reference.md) for the
Phase 3B claims-only render this was built from, and
[`sample_claims.json`](sample_claims.json) for the underlying claims.
