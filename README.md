...
`statistics` (`paperscope stats`), `evidence` (`paperscope evidence`), and structured
generation (`paperscope export-prompt` / `render`, plus an optional `[llm]`-gated
`generate`) are built — deterministic, offline, no API keys needed for the primary path;
see [`docs/statistics_and_evidence.md`](docs/statistics_and_evidence.md) and
[`docs/generation.md`](docs/generation.md). `skill build` isn't built yet — shown here so
the diagram doesn't overclaim what exists today. See
...
`stats`, `evidence`, `export-prompt`, and `render` are all deterministic and offline (no
OpenReview or Anthropic credentials needed); only the optional `generate` command talks
to a provider. See [`docs/statistics_and_evidence.md`](docs/statistics_and_evidence.md)
and [`docs/generation.md`](docs/generation.md) for the full command reference, output
...
review generated by the current skill, and a worked structured-generation example
(`sample_claims.json` → `sample_reference.md`, see [`docs/generation.md`](docs/generation.md)).

---

## Updating the Skill (the learning loop)

The pre-built skill files in `skill/references/` are still calibrated by hand today —
wiring validated generation output directly into `skill/references/*.md` is Phase 4
(`build-skill`), not built yet. Two ways to update them right now:

**Manual (original, still supported):**
...
**Structured, evidence-traced (Phase 3B, new):** `paperscope stats` →
`paperscope evidence` → `paperscope export-prompt` → run the exported prompt through
Claude Code by hand → `paperscope render` produces a validated, fully-cited Markdown
draft (every claim traces to a real excerpt or statistic — see
[`docs/generation.md`](docs/generation.md)) that a human still merges into
`skill/references/*.md` manually for now.
